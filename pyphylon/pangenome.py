...
def build_cds_pangenome(
    genome_faa_paths, output_dir, name="Test", cdhit_args={"-n": 5, "-c": 0.8}, fastasort_path=None, save_csv=True
):
    """
    ...
    output_shared_headers = (
        output_dir + "/" + name + "_redundant_headers.tsv"
    ...
    df_alleles, df_genes = build_genetic_feature_tables(
        output_nr_clstr, genome_faa_paths, name, cluster_type="cds", header_to_allele=header_to_allele
    )
...
def build_cds_nucl_pangenome(
    ...
    output_shared_headers = (
        output_dir + "/" + name + "_coding_nuc_redundant_headers.tsv"
    )  # records headers that have the same sequence
    output_missing_headers = (
        output_dir + "/" + name + "_coding_nuc_missing_headers.txt"
    ...
    """ Process gene/allele membership into binary tables """
    df_alleles, df_genes = build_genetic_feature_tables(
        output_nr_clstr, genome_coding_paths, name, cluster_type="cds", header_to_allele=header_to_allele
    )
...
def build_noncoding_pangenome(
    ...
    for i, gff_fna in enumerate(genome_data):
        ...
        extract_noncoding(genome_gff, genome_fna, genome_nc, flanking=flanking, allowed_features=allowed_features)
    ...
    nr_seq_hashes, missing_headers = consolidate_seqs(
        genome_noncoding_paths, output_nr_fna, output_shared_headers, output_missing_headers
    )
    # maps sequence hash to headers of that sequence, in order observed
    ...
    output_allele_names = (
        output_dir + "/" + name + "_noncoding_allele_names.tsv"
    )  # allele names vs non-redundant headers
    output_allele_names = output_allele_names.replace("//", "/")
    header_to_allele = rename_genes_and_alleles(
        output_nr_clstr,
        output_nr_fna,
    ...
    df_nc_alleles, df_nc_genes = build_genetic_feature_tables(
        output_nr_clstr, genome_noncoding_paths, name, cluster_type="noncoding", header_to_allele=header_to_allele
    )
...
def build_genetic_feature_tables(
    clstr_file, genome_fasta_paths, name="Test", cluster_type="cds", shared_header_file=None, header_to_allele=None
):
    """
    ...
    header_to_allele = load_header_to_allele(clstr_file, shared_header_file, header_to_allele, cluster_type)
    ...
    for i, genome_fasta in enumerate(sorted(genome_fasta_paths)):
        ...
        with open(genome_fasta, "r") as f_fasta:
            header = ""
            seq = ""  # track the sequence to skip over empty sequences
...
def load_header_to_allele(
    clstr_file=None, shared_header_file=None, header_to_allele=None, name="Test", cluster_type="cds"
):
    """
...
def build_downstream_pangenome(
    genome_data,
    allele_names,
    output_dir,
    limits=(-3, 50),
...
def extract_downstream_sequences(
    genome_gff,
    genome_fna,
    downstream_out,
    limits=(-3, 50),
...
def extract_proximal_sequences(
    genome_gff,
    genome_fna,
    proximal_out,
    limits,
    ...
    def extract_utr(start, stop, strand, contig, contig_seq, occupancy):
        ...
        pos = (side, strand)
        utr_side = (
            start if pos in [("upstream", "+"), ("downstream", "-")] else stop
        )  # side UTR effectively starts from
                    ...
                    if contig in contigs:
                        if gffid in feat_to_allele or feat_to_allele is None:
                            try:
                                ...
                                continue
                            proximal, is_fragment = extract_utr(
                                start, stop, strand, contig, contig_seq, strand_occupancy
                            )
        ...
        with open(genome_gff, "r") as f_gff:
            for line in f_gff:
                ...
                if not line[0] == "#" and not len(line.strip()) == 0:
                    contig, src, feature_type, start, stop, score, strand, phase, meta = line.split("\t")
...
def validate_upstream_table(df_upstream, upstream_fna_paths, nr_upstream_fna, allele_names, log_group=1):
    """
    ...
    return validate_table_against_fasta(
        df_features=df_upstream,
        genome_fasta_paths=upstream_fna_paths,
        features_fasta=nr_upstream_fna,
        allele_names=allele_names,
        log_group=log_group,
    )


def validate_downstream_table(df_downstream, downstream_fna_paths, nr_downstream_fna, allele_names, log_group=1):
    """
    ...
    """
    return validate_table_against_fasta(
        df_features=df_downstream,
        genome_fasta_paths=downstream_fna_paths,
        features_fasta=nr_downstream_fna,
        allele_names=allele_names,
        log_group=log_group,
    )


def validate_allele_table(df_alleles, genome_fasta_paths, alleles_fasta, log_group=1):
    """
    ...
    return validate_table_against_fasta(
        ...
        genome_fasta_paths=genome_fasta_paths,
        features_fasta=alleles_fasta,
        allele_names=None,
        log_group=log_group,
    )


def validate_table_against_fasta(df_features, genome_fasta_paths, features_fasta, allele_names=None, log_group=1):
    """
    ...
    for i, genome_fasta in enumerate(sorted(genome_fasta_paths)):
        ...
        with open(genome_fasta, "r") as f_fasta:
            ...
            for line in f_fasta:
                if line[0] == ">":  # new sequence encountered
                    genome_features = check_genome_sequence(
                        seq_blocks, genome_features, feature_header, missing_features
                    )
...
def validate_upstream_table_direct(df_upstream, genome_fna_paths, nr_upstream_fna, limits=(-50, 3), log_group=1):
    """
    ...
    """
    validate_proximal_table_direct(df_upstream, genome_fna_paths, nr_upstream_fna, limits, "upstream", log_group)


def validate_downstream_table_direct(df_downstream, genome_fna_paths, nr_downstream_fna, limits=(-3, 50), log_group=1):
    """
    ...
    """
    validate_proximal_table_direct(df_downstream, genome_fna_paths, nr_downstream_fna, limits, "downstream", log_group)
...
def generate_annotations(features, annotation_files):
    ...
    for feature in features:
        name, cluster_type, cluster_num, variant_type, variant_num = breakdown_feature_name(feature)
...
def extract_coding_fna(genome_gff, genome_fna, coding_out, allowed_features=["CDS", "tRNA"]):
    """
...
def find_pangenome_segments(df_genes, threshold=0.1, ax=None):
    ...
    dual_power_cdf = lambda x, c1, c2, a1, a2, k: Y[0] * (
        c1 * np.power(x, 1.0 - a1) / (1.0 - a1) - c2 * np.power(n - x, 1.0 - a2) / (1.0 - a2) + k
    )
...
def estimate_pan_core_size(df_genes, num_iter=10, log_batch=1):
    ...
    df_pan_core = pd.DataFrame(
        index=iter_index,
        columns=pan_cols + core_cols + acc_cols + rare_cols,
...
def fit_heaps(df_freqs):
    ...
    popt, pcov = scipy.optimize.curve_fit(heaps, np.arange(1, n_strains + 1), df_freqs.values, p0=p0)
    return popt
...
from operator import add, eq, ge, itemgetter, le, mul, ne
...
def argmax(X, axis=None):
    ...
    if axis is None:
        ...
        eX = [X[idxX[0, idx], col] for idx, col in zip(range(X.shape[1]), range(X.shape[1]))]
    else:
        eX = [X[row, idxX[idx, 0]] for row, idx in zip(range(X.shape[0]), range(X.shape[0]))]
    return eX, idxX
