...
def run_densmap(data: pd.DataFrame, low_memory: bool = False, n_neighbors: int = None):
    """
...
def run_hdbscan(
    embedding: np.ndarray, max_range: Optional[int] = None, core_dist_n_jobs: int = 8
    ...
    index = pd.MultiIndex.from_product(
        iterables=[min_cluster_sizes, min_samples_range], names=["min_cluster_size", "min_samples"]
    )
...
class NmfModel(object):
    ...
    def __init__(self, data: pd.DataFrame, ranks: Iterable, max_iter: int = 10_000) -> None:
        """
    ...
    def P_reconstructed_dict(self):
        ...
        if not self._P_reconstructed_dict:
            reconstr, error, confusion = generate_nmf_reconstructions(
                self.data, self.L_binarized_dict, self.A_binarized_dict
            )
    ...
    def df_metrics(self):
        ...
        if not self._df_metrics:
            df_metrics = calculate_nmf_reconstruction_metrics(self.P_reconstructed_dict, self.P_confusion_dict)
...
class PVGE(object):
    ...
    def densmap(self):
        if not self._densmap:
            self._densmap, self._embedding = run_densmap(self._data, self._low_memory, self._n_neighbors)
    ...
    def hdbscan(self):
        if not self._hdbscan_best_model:
            best_model, best_labels, best_model_sil_score, models_df = run_hdbscan(
                self._embedding, self._max_range, self._core_dist_n_jobs
            )
    ...
    def run_densmap(self, low_memory: bool = False):
        ...
        densmap, embedding = run_densmap(self._data, self._low_memory, self._n_neighbors)
    ...
    def run_hdbscan(self, max_range: int = None, core_dist_n_jobs: int = None):
        ...
        logger.debug(f"Running HDBSCAN with max_range={self._max_range}, " f"core_dist_n_jobs={self._core_dist_n_jobs}")
        best_model, best_labels, best_model_sil_score, models_df = run_hdbscan(
            self.embedding, self._max_range, self._core_dist_n_jobs
        )
        self._hdbscan_best_model = best_model
        self._labels = best_labels
        self._hdbscan_best_model_sil_score = best_model_sil_score
